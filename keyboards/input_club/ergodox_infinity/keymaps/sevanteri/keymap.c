/* vim: set fdm=marker */
// Nordic layout for Ergodox infinity

#include QMK_KEYBOARD_H
#include "debug.h"
#include "action_layer.h"
#include "action.h"
#include "version.h"
#include "keymap_finnish.h"
#include "sevanteri.h"
#include "process_combo.h"
#include "wrappers.h"

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
/* Basic layer {{{
 *
 *
 * ,--------------------------------------------------.           ,--------------------------------------------------.
 * |  ISO/  |   1  |   2  |   3  |   4  |   5  |      |           |      |   6  |   7  |   8  |   9  |   0  |   +    |
 * |--------+------+------+------+------+-------------|           |------+------+------+------+------+------+--------|
 * |  Tab   |   Q  |   W  |   E  |   R  |   T  |  ¨^  |           |  Å   |   Y  |   U  |   I  |   O  |   P  |   '    |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * |  ESC   |   A  |   S  |   D  |   F  |   G  |------|           |------|   H  |   J  |   K  |   L  |   Ö  |   Ä    |
 * |--------+------+------+------+------+------| DEL  |           | BSPC |------+------+------+------+------+--------|
 * | LShift<|   Z  |   X  |   C  |   V  |   B  |      |           |      |   N  |   M  |   ,  |   .  |  -   | RShift |
 * `--------+------+------+------+------+-------------'           `-------------+------+------+------+------+--------'
 *   | Ctrl |  ´`  |  §½  | RBTN | LBTN |                                       | _NUM | PRSC | INS  | RALT | SWAP |
 *   `----------------------------------'                                       `----------------------------------'
 *                                        ,-------------.       ,-------------.
 *                                        | HOME | END  |       | PgUp |RCTRL |
 *                                 ,------+------+------|       |------+--------+------.
 *                                 | LATL |      |FUNCL |       | PgDn |  FUNC  | Shift|
 *                                 | Space| BSPC |------|       |------|  Enter | Space|
 *                                 |      |      | LCTRL|       |FUNCL |        |      |
 *                                 `--------------------'       `----------------------'
 */

// layer 0 : default
[_BASE] = LAYOUT_ergodox_pretty_wrapper(

    __NUM_L2_________________,  TG(_GAME),  /*****/  _______,  __________________NUM_R2_,
    _QWERTY_L1_______________,  _______,    /*****/  FI_ARNG,  _______________QWERTY_R1_,
    _QWERTY_L2_______________,  /**/        /*****/  /**/      _______________QWERTY_R2_,
    _QWERTY_L3_______________,  KC_DEL,     /*****/  KC_BSPC,  _______________QWERTY_R3_,
    KC_NO,  FI_ACUT,  _______,  ____QWERTY_L4___,  /*****/  ____QWERTY_R4___,  KC_INS,  KC_RALT,  KC_NO,

    /******/  /******/ /******/ /******/ /******/ KC_HOME, KC_END,    /*****/ KC_PGUP,   KC_RCTL,
    /******/  /******/ /******/ /******/ /******/ /******/ _______,   /*****/ KC_PGDN,
    /******/  /******/ /******/ /******/ _QWERTY_THUMBL__, KC_LCTL,  /*****/ MO(_FUNC), _QWERTY_THUMBR__

),
// }}}
/* Number and symbol layer {{{
 *
 *
 *
 * ,--------------------------------------------------.           ,--------------------------------------------------.
 * |        |      |      |      |      |      |      |           |      |      |      |      |      |      |        |
 * |--------+------+------+------+------+-------------|           |------+------+------+------+------+------+--------|
 * |   §    |  1   |  2   |  3   |  4   |  5   |  ~   |           |      |  6   |  7   |  8   |  9   |  0   |   +    |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * |   ½    |  !   |  "   |  #   |  ¤   |  %   |------|           |------|  &   |  /   |  (   |  )   |  =   |   ?    |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * |   /    |      |  @   |  £   |  $   |  ‰   |      |           |      |  ‚   |  {   |  [   |  ]   |  }   |   \    |
 * `--------+------+------+------+------+-------------'           `-------------+------+------+------+------+--------'
 *   |      |      |       |     | CODE |                                       |      |  ´   |      |      |      |
 *   `----------------------------------'                                       `----------------------------------'
 *                                        ,-------------.       ,-------------.
 *                                        |      |      |       |      |      |
 *                                 ,------+------+------|       |------+--------+------.
 *                                 |      |      |      |       |      |        |      |
 *                                 |  .   |      |------|       |------|        |      |
 *                                 |      |      |      |       |      |        |      |
 *                                 `--------------------'       `----------------------'
 */

// number and symbol layer
[_NUM] = LAYOUT_ergodox_pretty_wrapper(

    __NUM_L1_________________, _______,       /*****/ _______, __________________NUM_R1_,
    __NUM_L1_________________, RALT(FI_DIAE), /*****/ _______, __________________NUM_R1_,
    __NUM_L2_________________, /******/       /*****/ /******/ __________________NUM_R2_,
    __NUM_L3_________________, _______,       /*****/ _______, __________________NUM_R2_,
    _______,       _______, _______, _____NUM_L4_____, /******/ /******/       /*****/ /******/ /******/ _____NUM_R4_____, _______, _______, _______,

    /******/       /******/ /******/ /******/ /******/ _______, _______,       /*****/ _______, _______,
    /******/       /******/ /******/ /******/ /******/ /******/ _______,       /*****/ _______,
    /******/       /******/ /******/ /******/ ____NUM_THUMBL__, _______,       /*****/ _______, ____NUM_THUMBR__

),
// }}}
/* Function layer {{{
 *
 *
 *
 * ,--------------------------------------------------.           ,--------------------------------------------------.
 * |        |      |      |      |      |      |      |           |      |      |      |      |      |      |        |
 * |--------+------+------+------+------+-------------|           |------+------+------+------+------+------+--------|
 * |        |  F1  |  F2  |  F3  |  F4  |      | GAME |           |      |BRBAK | CTAB |CSTAB |BRFWD |VOLUP |        |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * |        |  F5  |  F6  |  F7  |  F8  |      |------|           |------| LEFT | DOWN |  UP  |RIGHT |VOLDN |        |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * |        |  F9  |  F10 |  F11 |  F12 |      |      |           |      | MLEFT| MDOWN| MUP  |MRIGHT| PLAY |        |
 * `--------+------+------+------+------+-------------'           `-------------+------+------+------+------+--------'
 *   |      |      |       |     |      |                                       |      |      |      |      |      |
 *   `----------------------------------'                                       `----------------------------------'
 *                                        ,-------------.       ,-------------.
 *                                        |      |BLTOG |       |      |      |
 *                                 ,------+------+------|       |------+--------+------.
 *                                 |      |      | BL+  |       |      |        |      |
 *                                 |      |      |------|       |------|        |      |
 *                                 |      |      | BL-  |       |      |        |      |
 *                                 `--------------------'       `----------------------'
 */

// layer 1 : functions
[_FUNC] = LAYOUT_ergodox_pretty_wrapper(

    _______, _______, _______, _______, _______, _______, _______,  /*****/ _______, _______, _______, _______, _______, _______, _______,
    _______, _________________FUNCL_L1__________________, TG(_GAME), /*****/ _______, _________________FUNCL_R1__________________, _______,
    _______, _________________FUNCL_L2__________________, /******/  /*****/ /******/ _________________FUNCL_R2__________________, _______,
    _______, _________________FUNCL_L3__________________, _______,  /*****/ _______, _________________FUNCL_R3__________________, _______,
    _______, _______, _______, ____FUNC_L4_____, /******/ /******/  /*****/ /******/ /******/ ____FUNC_R4_____, _______,  _______, _______,

    /******/ /******/ /******/ /******/ /******/ _______, BL_TOGG,  /*****/ _______, _______,
    /******/ /******/ /******/ /******/ /******/ /******/ BL_UP,    /*****/ _______,
    /******/ /******/ /******/ /******/ ___FUNC_THUMBL__, BL_DOWN,  /*****/ _______, ___FUNC_THUMBR__

),
// }}}
/* Gaming {{{ */

[_GAME] = LAYOUT_ergodox_pretty_wrapper(

    KC_ESC,   KC_0,      KC_1,     KC_2,   KC_3,      KC_4,    KC_5,    /*****/  TG(_GAME),  _______,  _______,  _______,  _______,  _______,  _______,
    KC_T,     KC_TAB,    KC_Q,     KC_W,   KC_E,      KC_R,    KC_6,    /*****/  S(KC_F1),   _______,  _______,  _______,  _______,  _______,  _______,
    KC_G,     KC_LCTL,  KC_A,     KC_S,   KC_D,      KC_F,    /**/     /*****/  /**/        _______,  _______,  _______,  _______,  _______,  _______,
    KC_B,     KC_LSFT,   KC_Z,     KC_X,   KC_C,      KC_V,    KC_7,    /*****/  KC_BSPC,    _______,  _______,  _______,  _______,  _______,  _______,
    KC_LALT,  KC_LEFT,   KC_DOWN,  KC_UP,  KC_RIGHT,  /*****/  /*****/  /*****/  /*****/     /*****/   KC_VOLU,  KC_VOLD,  _______,  _______,  KC_RGUI,

    /**/      /**/     /**/     /**/     /**/     KC_8,     KC_9,          /*****/ _______, _______,
    /**/      /**/     /**/     /**/     /**/     /**/      SH_OS,         /*****/ _______,
    /**/      /**/     /**/     /**/     KC_SPC,  KC_LSFT,  KC_ENT,        /*****/ _______, KC_ENT, _______

)
// }}}
/* Mouse {{{
 *
 *
 *
 * ,--------------------------------------------------.           ,--------------------------------------------------.
 * |        |      |      |      |      |      |      |           |      |      |      |      |      |      |        |
 * |--------+------+------+------+------+-------------|           |------+------+------+------+------+------+--------|
 * |        |      |      |      |      |      |      |           |      |      |      |      |      |      |        |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * |        |      |      |      |      |      |------|           |------|      |      |      |      |      |        |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * |        |      |      |      |      |      |      |           |      |      |      |      |      |      |        |
 * `--------+------+------+------+------+-------------'           `-------------+------+------+------+------+--------'
 *   |      |      |       |     |      |                                       |      |      |      |      |      |
 *   `----------------------------------'                                       `----------------------------------'
 *                                        ,-------------.       ,-------------.
 *                                        |      |      |       |      |      |
 *                                 ,------+------+------|       |------+--------+------.
 *                                 |      |      |      |       |      |        |      |
 *                                 |      |      |------|       |------|        |      |
 *                                 |      |      |      |       |      |        |      |
 *                                 `--------------------'       `----------------------'
 */
// ,[_LAYER] = LAYOUT_ergodox_pretty_wrapper(
// 
//     _______,  _______, _______, _______, _______, _______, _______, /*****/ _______, _______, _______, _______, _______, _______, _______,
//     _______,  _______, _______, _______, _______, _______, _______, /*****/ _______, _______, _______, _______, _______, _______, _______,
//     _______,  _______, _______, _______, _______, _______, /**/     /*****/ /**/     _______, _______, _______, _______, _______, _______,
//     _______,  _______, _______, _______, _______, _______, _______, /*****/ _______, _______, _______, _______, _______, _______, _______,
//     _______,  _______, _______, _______, _______, /**/     /**/     /*****/ /**/     /**/     _______, _______, _______, _______, _______,
// 
//     /**/      /**/     /**/     /**/     /**/     _______, _______, /*****/ _______, _______,
//     /**/      /**/     /**/     /**/     /**/     /**/     _______, /*****/ _______,
//     /**/      /**/     /**/     /**/     _______, _______, _______, /*****/ _______, _______, _______
// 
// )
// }}}

};

layer_state_t layer_state_set_user(layer_state_t state) {
    combo_enable();
    if (layer_state_cmp(state, _GAME)) {
        combo_disable();
    }
    return state;
};
