...
#define do_(code) do { code } while (0)

#define h do_(_stap(KC_LEFT);)
#define l do_(_stap(KC_RIGHT);)

#define j _stap(KC_DOWN)
#define k _stap(KC_UP)

#define e do_(stap(LCTL(KC_RIGHT),pressed); )
#define b do_(stap(LCTL(KC_LEFT),pressed); )
#define end do_(_stap(KC_END);)
#define home do_(_stap(KC_HOME);)

#define vdn do_(select_on = true;)
#define vup do_(select_on = false;)
#define v(s) do_( vdn; s; vup; )

#define V do_( vdn; tap_code(KC_HOME); end; )

#define _X tap_code(KC_BSPC)
#define x  tap_code(KC_DEL)
...
void* leader_start_func(uint16_t keycode, bool pressed) {
    mods = get_mods();
#define WITH_SFT(c) \
    do_(if (mods & MOD_MASK_SHIFT) {del_mods(MOD_MASK_SHIFT); c; break;})
#define WITH_CTRL(c) \
    do_(if (mods & MOD_MASK_CTRL) {del_mods(MOD_MASK_CTRL); c; break;})
    ...
    switch (keycode) {
        ...
        case KC_X:
            (mods & MOD_MASK_SHIFT) ? tap(KC_BSPC, pressed): tap(KC_DEL, pressed);
            TO_NORMAL;
    ...
    switch (keycode) {
        ...
        case KC_S:
            WITH_SFT(do_(V; x; TO_INSERT;));
            x; TO_INSERT;

        case KC_C:
            WITH_SFT(do_(vdn; end; x; TO_INSERT;));
            if (!pending_operator) {
                if (select_on) {
                    x;
                    TO_INSERT;
        ...
        case KC_D:
            WITH_SFT(do_(vdn; end; x; TO_NORMAL;));
            if (!pending_operator) {
                if (select_on) {
                    x;
                    TO_NORMAL;
                } else {
                    return await_operator(KC_DEL, NORMAL);
                }
            } else {
                vup; V;
            }
        ...
        case KC_V:
            WITH_SFT(V; return NORMAL;);
            if (select_on) vup;
            else vdn;
            return NORMAL;

        case KC_A:
            WITH_SFT(do_(vup; end; TO_INSERT;));
            l; TO_INSERT;
        case KC_I:
            WITH_SFT(do_(vup; home; TO_INSERT;));
            TO_INSERT;
