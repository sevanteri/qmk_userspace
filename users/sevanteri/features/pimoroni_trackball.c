...
bool trackball_get_interrupt(void) {
    ...
    uint8_t data[1] = {};
    i2c_read_register(TRACKBALL_WRITE, REG_INTERRUPT_PIN, data, 1, TB_I2C_TIMEOUT);
...
void trackball_read_state(uint8_t* data, uint16_t size_of_data) {
    i2c_read_register(TRACKBALL_WRITE, REG_LEFT, data, size_of_data, TB_I2C_TIMEOUT);
}
...
void trackball_set_brightness(uint8_t brightness) {
    uint8_t data[4] = {};
    i2c_read_register(TRACKBALL_WRITE, REG_RED, data, 4, TB_I2C_TIMEOUT);
    for (int i=0; i<4; i++) {
        if (data[i]) {
            data[i] = brightness;
        }
    }
    i2c_write_register(TRACKBALL_WRITE, REG_RED, data, 4, TB_I2C_TIMEOUT);
}
